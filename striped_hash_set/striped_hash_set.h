...
private:
	...
	mutable std::vector<std::mutex> stripes_mtx;
	...
	size_t get_stripe_index(size_t hash_value)const {
		return hash_value % stripes_mtx.size();
	}
...
Hash striped_hash_set<T, Hash>::hash;

template <class T, class Hash>
void striped_hash_set<T, Hash>::add(const T & v) {
	auto h = hash(v);
	std::unique_lock<std::mutex> lck(stripes_mtx[get_stripe_index(h)]);
	std::forward_list<T> & bucket = buckets[get_bucket_index(h)];
	if (std::find(bucket.begin(), bucket.end(), v) == bucket.end()) {
		bucket.push_front(v);
		//std::cout << v << ' ' << get_stripe_index(h) << std::endl;
		size.fetch_add(1);
	}
	if (((float)size.load() / buckets.size()) > load_factor) {
		size_t num_buckets = buckets.size();
		lck.unlock();
		expand_set(num_buckets);
	}
}
...
void striped_hash_set<T, Hash>::expand_set(size_t num_buckets) {
	...
	for (auto it = buckets.begin(); it != buckets.end(); ++it) {
		for (auto it_in_bucket = it->begin(); it_in_bucket != it->end(); ++it_in_bucket) {
			...
			new_buckets[h % new_buckets.size()].push_front(*it_in_bucket);
		}
	}
...
bool striped_hash_set<T, Hash>::contains(const T & v)const {
	auto h = hash(v);
	auto stripe_index = get_stripe_index(h);
	std::unique_lock<std::mutex> lock(stripes_mtx[stripe_index]);
	const std::forward_list<T> & bucket = buckets[get_bucket_index(h)];
	if (std::find(bucket.begin(), bucket.end(), v) == bucket.end())
		return false;
	return true;
}
